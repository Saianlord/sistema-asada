<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">
            Registrar Iniciativa de Proyecto
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-slate-200 p-8">
                <form method="POST" action="{{ route('projects.store') }}">
                    ...
                    <div class="mt-6">
                        ...
                        <textarea id="description" name="description" rows="5" class="block mt-1 w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-slate-700" required>{{ old('description') }}</textarea>
                    ...
                    <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="criticality" name="criticality" class="block mt-1 w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-slate-700" required>
                                <option value="">Selecciona criticidad</option>
                                <option value="low" {{ old('criticality') === 'low' ? 'selected' : '' }}>Baja</option>
                                <option value="medium" {{ old('criticality') === 'medium' ? 'selected' : '' }}>Media</option>
                    ...
                    <div class="flex items-center justify-end mt-8 space-x-4">
                        <a href="{{ route('projects.index') }}" class="inline-flex items-center px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm font-medium rounded-lg transition-colors">
                            Cancelar
                        </a>
